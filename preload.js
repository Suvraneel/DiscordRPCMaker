...
let latestpath = os.platform() == "win32" ? opendir + "\\" + "latest.txt" : dir + "/" + "latest.txt"
...
function saveAsJson() {
	...
	let buttononelabel = document.getElementById("button1-input-name").value.toString().replaceAll("-", " ").replaceAll("_", " ")
	...
	let buttontwolabel = document.getElementById("button2-input-name").value.toString().replaceAll("-", " ").replaceAll("_", " ")
...
document.addEventListener("DOMContentLoaded", () => {
	...
	if (settings.autostart) { document.getElementById("autostart-btn").click() }
	...
	if (settings.autostart == true) {
		...
		let latestpresence = JSON.parse(fs.readFileSync(dir + latest + ".json", 'utf8'));
